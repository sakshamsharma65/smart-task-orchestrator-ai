...
const AiTaskCreationSheet: React.FC<Props> = ({ open, onOpenChange, onTaskCreated, currentUserId }) => {
  ...
  const createTask = async () => {
    ...
    try {
      ...
      const normalizedType = mergedTask.type === "personal" ? "personal" : "team";
      ...
      const payload = {
        ...
        start_date: mergedTask.start_date ?? today,
        due_date: mergedTask.due_date ?? null,
        type: normalizedType,
        created_by: currentUserId ?? mergedTask.assigned_to,
        assigned_to: mergedTask.assigned_to,
        estimated_hours:
          typeof mergedTask.estimated_hours === "number" ? mergedTask.estimated_hours : null,
        team_id: normalizedType === "team" ? mergedTask.team_id ?? null : null,
        project_id: mergedTask.project_id ?? null,
        milestone_id: mergedTask.milestone_id ?? null,
        feature_id: mergedTask.feature_id ?? null,
        is_time_managed: false,
        todos_enabled: mergedTask.todos_enabled ?? false,
        todo_group_id: mergedTask.todo_group_id ?? null,
      };
