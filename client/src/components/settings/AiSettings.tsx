...
- Required fields: Title, Assigned To, Due Date, Estimated Hours.
- Optional fields: Description, Priority (Critical/High/Medium/Low/Minimal), Type (Personal/Team).
...
const handleTest = async () => {
  if (!form.api_key && form.provider !== "ollama") {
    toast({
      title: "Enter an API key first before testing",
      variant: "destructive",
    });
    return;
  }
  ...
  try {
    // Don't send masked keys like ••••
    const isMaskedKey = /^[•*]+/.test(form.api_key);

    await apiClient.post("/ai/test-connection", {
      provider: form.provider,
      api_key: isMaskedKey ? "" : form.api_key,
      model: form.model,
