...
export default function CreateDefectSheet({ open, onOpenChange, currentUserId, defaultProjectId, defaultProjectName }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if(!form.steps_to_reproduce.trim()){
      toast({ title: "Steps to Reproduce required", variant: "destructive" });
      return;
    }
    if(!form.expected_behavior.trim()){
      toast({ title: "Expected Behavior required", variant: "destructive" });
      return;
    }
    if(!form.actual_behavior.trim()){
      toast({ title: "Actual Behavior required", variant: "destructive" });
      return;
    }
    if(!currentUserId){
