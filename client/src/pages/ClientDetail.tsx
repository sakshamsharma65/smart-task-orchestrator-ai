...
console.log(" sakkuallAccesses =>", allAccesses);
  // Contact mutations
  ...
  const updateClientMutation = useMutation({
    mutationFn: (data: any) => apiClient.patch(`/clients/${id}`, data),
    onSuccess: () => {
    ...
    onError: (e: any) => toast({ title: "Error", description: e?.message || "Failed to update client.", variant: "destructive" }),
  });
  ...
  const openEditContact = (c: any) => {
    setEditContact(c);
    setContactForm({ name: c.name, email: c.email, phone: c.phone || "", job_title: c.job_title || "", is_active: c.is_active ?? true, password: "" });
    setContactDialog(true);
  };
  ...
  const handleSaveClient = () => {
    ...
    const payload = {
      ...
      organization_type: clientForm.organization_type || null,
      industry: clientForm.industry || null,
      primary_contact_name: clientForm.primary_contact_name || null,
  ...
  const ACCESS_LEVEL_WEIGHT: Record<string, number> = { observer: 1, collaborator: 2, approver: 3 };

  const selectedProjectContactIds = selectedProject
    ? (allAccesses as any[]).filter((a: any) => a.project_id === selectedProject.id).map((a: any) => a.contact_id)
    : [];

  const availableContacts = selectedProject
    ? (contacts as any[]).filter((c: any) => !selectedProjectContactIds.includes(c.id))
    : (contacts as any[]);

  const getHighestAccessLevelForContact = (contactId: string) => {
    const contactAccesses = (allAccesses as any[]).filter((access: any) => access.contact_id === contactId);
    if (contactAccesses.length === 0) return "observer";

    return contactAccesses.reduce((highest: string, access: any) => {
      const nextLevel = access.access_level || "observer";
      return ACCESS_LEVEL_WEIGHT[nextLevel] > ACCESS_LEVEL_WEIGHT[highest] ? nextLevel : highest;
    }, "observer");
  };
  ...
  useEffect(() => {
    if (!client) return;
    if (searchParams.get("edit") === "1" && !clientDialog) {
      openClientEdit();
    }
  }, [client, contacts, searchParams, clientDialog]);
  ...
  return (
    <div className="p-6 space-y-6">
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y dark:divide-gray-700">
                  {(contacts as any[]).map((c: any) => {
                    const highestAccessLevel = getHighestAccessLevelForContact(c.id);
                    const lvl = ACCESS_LEVEL_CONFIG[highestAccessLevel] || ACCESS_LEVEL_CONFIG.observer;
                    const Icon = lvl.icon;
                    return (
                      <tr key={c.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors">
                        ...
                        <td className="px-4 py-3 text-xs text-gray-400">
                          {c.last_login_at ? formatOrgDate(c.last_login_at) : "Never"}
                        </td>
                        <td className="px-4 py-3">
      ...
      {/* ── Contact Dialog ── */}
      <Dialog open={clientDialog} onOpenChange={(open) => { if (!open) closeClientDialog(); }}>
        <DialogContent className="max-w-xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 py-2">
            <div className="rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
              ...
              <div className="p-4 grid grid-cols-2 gap-3">
                <div className="col-span-2 space-y-1">
                  ...
                  <Input className="h-10" value={clientForm.name} onChange={(e) => setClientForm((prev) => ({ ...prev, name: e.target.value }))} />
                </div>
                <div className="space-y-1">
                  <Label className="text-xs font-semibold">Organisation Type</Label>
                  <Select value={clientForm.organization_type || "none"} onValueChange={(value) => setClientForm((prev) => ({ ...prev, organization_type: value === "none" ? "" : value }))}>
                    <SelectTrigger className="h-10"><SelectValue placeholder="Select type" /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="none">None</SelectItem>
                      {ORG_TYPES.map((type) => <SelectItem key={type} value={type}>{type}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-1">
                  <Label className="text-xs font-semibold">Industry</Label>
                  <Select value={clientForm.industry || "none"} onValueChange={(value) => setClientForm((prev) => ({ ...prev, industry: value === "none" ? "" : value }))}>
            ...
            <div className="rounded-xl border border-green-200 dark:border-green-800/50 overflow-hidden">
              ...
              <div className="p-4 grid grid-cols-2 gap-3">
                <div className="col-span-2 space-y-1">
                  ...
                  <Input className="h-10" value={clientForm.primary_contact_name} onChange={(e) => setClientForm((prev) => ({ ...prev, primary_contact_name: e.target.value }))} />
                </div>
                <div className="space-y-1">
                  <Label className="text-xs font-semibold">Email</Label>
                  <Input className="h-10" type="email" value={clientForm.email} onChange={(e) => setClientForm((prev) => ({ ...prev, email: e.target.value }))} disabled />
                </div>
                <div className="space-y-1">
                  <Label className="text-xs font-semibold">Phone</Label>
                  <Input className="h-10" type="tel" value={clientForm.phone} onChange={(e) => setClientForm((prev) => ({ ...prev, phone: e.target.value }))} />
                </div>
                <div className="space-y-1">
                  <Label className="text-xs font-semibold">Update Password (Optional)</Label>
                  <Input className="h-10" type="password" placeholder="Minimum 6 characters" value={clientForm.password} onChange={(e) => setClientForm((prev) => ({ ...prev, password: e.target.value }))} />
                </div>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-purple-200 dark:border-purple-800/50 border-dashed overflow-hidden">
              ...
              <div className="p-4">
                <Textarea rows={3} value={clientForm.notes} onChange={(e) => setClientForm((prev) => ({ ...prev, notes: e.target.value }))} />
              </div>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={closeClientDialog}>Cancel</Button>
            <Button className="bg-blue-600 hover:bg-blue-700 text-white" onClick={handleSaveClient} disabled={updateClientMutation.isPending}>
              Save Changes
            </Button>
          </DialogFooter>
        </DialogContent>
          ...
          <div className="space-y-4 py-2">
              ...
              <div className="p-4 grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <Input className="h-10" type="email" placeholder="jane@company.com" value={contactForm.email}
                    onChange={(e) => setContactForm(p => ({ ...p, email: e.target.value }))} disabled={!!editContact} />
                </div>
                <div className="space-y-1">
                  <Label className="text-xs font-semibold">Phone</Label>
            ...
            <div className="rounded-xl border border-green-200 dark:border-green-800/50 overflow-hidden">
              ...
              <div className="p-4 space-y-3">
                <div className="space-y-1">
                  <Label className="text-xs font-semibold">Project Access Controlled</Label>
            ...
            <div className="rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
              ...
              <div className="p-4 space-y-3">
                <div className="space-y-1">
                  ...
                  <Select value={accessForm.contact_id} onValueChange={(v) => {
                      setAccessForm(p => ({ ...p, contact_id: v }));
                      setSelectedProject(null);
                    }} disabled={!!editAccess}>
                ...
                {!editAccess && (
                  <div className="space-y-1">
                    ...
                    {(() => {
                      ...
                      const contactProjectIds = (allAccesses as any[])
                        .filter((a: any) => a.contact_id === accessForm.contact_id)
                        .map((a: any) => a.project_id);
                      const availableProjects = clientProjects.filter((p: any) => !contactProjectIds.includes(p.id));
                      ...
                      if (availableProjects.length === 0) {
                        return (
                          <div className="flex items-start gap-2 p-3 rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 text-xs text-amber-700 dark:text-amber-400">
                            <AlertCircle className="h-3.5 w-3.5 mt-0.5 shrink-0" />
                            <span>
                              This contact already has access to all client projects. Choose a different contact or edit existing project access.
                            </span>
                          </div>
                        );
                      }
                      ...
                      return (
                        <Select
                          ...
                          onValueChange={(v) => {
                            const project = availableProjects.find((p: any) => p.id === v);
                            setSelectedProject(project);
                            if (project && selectedProjectContactIds.includes(accessForm.contact_id)) {
                              setAccessForm(p => ({ ...p, contact_id: "" }));
                            }
                          }}
                        >
