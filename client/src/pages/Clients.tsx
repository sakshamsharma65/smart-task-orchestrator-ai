...
const { roles } = useCurrentUserRoleAndTeams();
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ["/api/clients"] });
      queryClient.invalidateQueries({ queryKey: ["/api/clients/all-contacts"] });
      queryClient.invalidateQueries({ queryKey: ["/api/clients", variables.id, "contacts"] });
...
const openEdit = (c: any, e: React.MouseEvent) => {
  ...
  setForm({ 
    ...
    notes: c.notes || "",
    password: "" // Keep blank for editing
  });
  setDialog(true);
};
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
            Open a client to grant project access and control portal rights like defect view, create, edit, and approval.
          </p>
        </div>
      {isAdmin && (  <Button onClick={openCreate} className="bg-blue-600 hover:bg-blue-700 text-white shrink-0">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="rounded-xl border border-green-200 dark:border-green-800/50 overflow-hidden">
              ...
              <div className="p-4 grid grid-cols-2 gap-3">
                ...
                {/* ---> ADD NEW FIELDS HERE <--- */}
                <div className="space-y-1">
                  <Label className="text-xs font-semibold">{editClient ? "Update Password (Optional)" : "Portal Password"}</Label>
                  <Input className="h-10" type="password" placeholder="Minimum 6 characters" value={form.password} onChange={(e) => setForm(p => ({ ...p, password: e.target.value }))} />
                </div>
              </div>
            </div>
