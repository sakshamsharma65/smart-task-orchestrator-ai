...
markEmailSettingsAsVerified(
  ...
  getProjectsByUserMembership(userId: string): Promise<Project[]>;
  getProjectsByManagedUsers(managerId: string): Promise<Project[]>;
  ...
  getProjectMembersWithUsers(projectId: string): Promise<(ProjectMember & { user_name?: string; email?: string })[]>;
...
export class DatabaseStorage implements IStorage {
  private getTaskSelectWithGroups() {
    return {
      ...getTableColumns(tasks),
      groups: sql`
        COALESCE(
          json_agg(
            DISTINCT jsonb_build_object('id', ${taskGroups.id}, 'name', ${taskGroups.name})
  ...
  private buildTasksWithGroupsQuery() {
    return db
      .select(this.getTaskSelectWithGroups())
      .from(tasks)
...
async getAllUsers(): Promise<any[]> { // Note: Return type is now 'any[]' or a custom type
  ...
  async getTask(id: string): Promise<Task | undefined> {
    const result = await this.buildTasksWithGroupsQuery()
...
async getTasksByUser(userId: string): Promise<Task[]> {
    return await this.buildTasksWithGroupsQuery()
      .where(or(eq(tasks.assigned_to, userId), eq(tasks.created_by, userId)))
      .groupBy(tasks.id)
...
async deleteTaskAttachment(id: string): Promise<void> {
 ...
 async getAllTeams(): Promise<Team[]> {
  ...
  const taskList = await this.buildTasksWithGroupsQuery()
...
async getTaskGroupsForUser(userId: string): Promise<TaskGroup[]> {
  ...
  async deleteProjectTemplate(id: string): Promise<void> {
        // Hard-delete all tasks linked to this project before removing the project
  ...
  // Add these to your storage class/object
async getProjectsByUserMembership(userId: string): Promise<Project[]> {
  // We join the projects table with project_members
  // If a row exists in project_members for this user, they see the project
  const result = await db
    .select({
      project: projects,
    })
    ...
    .where(and(eq(projectMembers.user_id, userId), eq(projectMembers.is_active, true)))
    .orderBy(desc(projects.created_at));

  return result.map(r => r.project);
}

async getProjectsByManagedUsers(managerId: string): Promise<Project[]> {
  const managedUsers = await db
    .select({ id: users.id })
    .from(users)
    .where(eq(users.manager, managerId));
  ...
  const result = await db
    ...
    .where(
      and(
        inArray(projectMembers.user_id, managedUserIds),
...
async isUserProjectMember(projectId: string, userId: string): Promise<boolean> {
  ...
  async getProjectMembersWithUsers(projectId: string): Promise<(ProjectMember & { user_name?: string; email?: string })[]> {
    const members = await db.select({
      ...
      .from(projectMembers)
      .leftJoin(users, eq(projectMembers.user_id, users.id))
      .where(and(eq(projectMembers.project_id, projectId), eq(projectMembers.is_active, true)))
      .orderBy(desc(projectMembers.joined_at));
