...
  password_hash: text("password_hash"), // <-- FIX: ADDED MISSING PASSWORD HASH FIELD
...
export const teamMemberships = pgTable(
  ...
  (table) => {
    return {
      uniqueTeamUser: unique("unique_team_user").on(
        table.team_id,
        table.user_id
      ),
    };
...
export const insertProjectSchema = createInsertSchema(projects).omit({
  ...
  budget_amount: z.union([z.number(), z.string().transform((s) => s ? parseInt(s) : null)]).nullable().optional(),
});
