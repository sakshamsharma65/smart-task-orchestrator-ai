...
async function initializeRoles() {
  ...
  try {
    ...
    if (existingRoles.length > 0) {
      console.log('⚠  Roles already exist in the database. Skipping role creation.');
    ...
    for (const roleConfig of BASE_ROLES) {
      try {
        ...
        console.log('⚠  Role ${roleConfig.name} already exists or failed to create');
      }
    }
    ...
    for (const role of createdRoles) {
      ...
      if (!permissionsConfig) {
        console.log('⚠  No permissions configuration found for role: ${role.name}');
        continue;
      }
